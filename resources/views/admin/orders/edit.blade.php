        ...
        <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white">
                    Edit Pesanan <span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-pink-500">#{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</span>
                </h1>
            </div>
            <a href="{{ route('admin.orders.show', $order->id) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-white border border-gray-200 px-3 py-1.5 text-xs font-bold text-gray-600 shadow-sm hover:bg-gray-50 transition-colors dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300">
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
                    ...
                    <div class="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm dark:border-gray-800 dark:bg-gray-dark">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="phone" name="phone" value="{{ old('phone', $order->phone) }}" required class="block w-full rounded-lg border-gray-300 bg-gray-50 py-2 px-3 text-xs font-semibold focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                            </div>

                            <!-- Delivery Date / Schedule -->
                            <div>
                                <label for="delivery_date" class="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-1.5">Jadwal Kirim / Ambil</label>
                                <input type="datetime-local" id="delivery_date" name="delivery_date" value="{{ old('delivery_date', $order->delivery_date ? \Carbon\Carbon::parse($order->delivery_date)->format('Y-m-d\TH:i') : '') }}" class="block w-full rounded-lg border-gray-300 bg-gray-50 py-2 px-3 text-xs font-semibold focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                            </div>

                            <!-- Address / Table -->
                            <div class="md:col-span-2">
                                <label for="shipping_address" class="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-1.5">Alamat Pengiriman / Nomor Meja</label>
                                <textarea id="shipping_address" name="shipping_address" rows="3" required class="block w-full rounded-lg border-gray-300 bg-gray-50 py-2 px-3 text-xs font-semibold focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 dark:border-gray-800 dark:bg-gray-900 dark:text-white">{{ old('shipping_address', $order->shipping_address) }}</textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-5">
                    ...
                    <div class="rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm dark:border-gray-800 dark:bg-gray-dark">
                        ...
                        <a href="{{ route('admin.orders.show', $order->id) }}" class="mt-2.5 block text-center border border-gray-200 dark:border-gray-800 dark:text-gray-400 dark:hover:bg-gray-850 hover:bg-gray-50 text-gray-600 font-bold py-2 px-4 rounded-lg text-xs transition-all">
                            Batal
                        </a>
                    </div>
                </div>
